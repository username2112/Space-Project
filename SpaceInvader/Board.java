	...
	public void animationCycle() {
			...
			/*if (bombc.isVisible()) {
				...
				for (Alien alien : aliens) {
					...
					if (alien.isVisible() && bombc.isVisible()) {
						...
						if (shotX >= (alienX) && bombcX <= (alienX + ALIEN_WIDTH) && bombcY >= (alienY)
								&& bombcY <= (alienY + ALIEN_HEIGHT)) {
		...
		for (Alien alien : aliens) {
			...
			if (alien.isVisible() && bombc.isVisible()) {
				...
				if (bombcX >= (alienX) && bombcX <= (alienX + ALIEN_WIDTH) && bombcY >= (alienY)
						&& bombcY <= (alienY + ALIEN_HEIGHT)) {
		...
		/*
		 * if (bombc.isVisible()) { int by = bombc.getY(); by += 6; // shot border if
		 * (by > BOARD_HEIGHT) { bombc.die(); } else { bombc.setY(by); } }
		 */
		// aliens
		for (Alien alien : aliens) {
			int x = alien.getX();
